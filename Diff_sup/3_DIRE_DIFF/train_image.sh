...
export EXP_NAME="img_adm"           # 实验名字（用于区分不同实验）
...
for BATCH_SIZE in "${BATCH_SIZES[@]}"; do
    for DATA in "${DATAS[@]}"; do
      ...
      # 构造路径（按你的约定）
      DATASET_ROOT="/root/autodl-tmp/Reconstruct/imagenet/${VIS_MODE}/${DATA}"
      EXP_ROOT="/root/autodl-tmp/result/img_adm/${BATCH_SIZE}/${VIS_MODE}/${DATA}/exp"
